...
func TestGetAll(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		expectedFlights := []models.Flight{
			{Id: 1, FromWhere: "NYC", Destination: "LAX", FlightTime: flightTime1, FlightDuration: 6},
			{Id: 2, FromWhere: "LAX", Destination: "NYC", FlightTime: flightTime2, FlightDuration: 6},
		}

		rows := sqlmock.NewRows([]string{"id", "from_where", "destination", "flight_time", "flight_duration"}).
...
func TestInsert(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		// Expect GetById to be called after insertion
		mock.ExpectQuery("SELECT \\* FROM " + os.Getenv("PSQL_TABLE_NAME") + " WHERE id = \\$1;").
			WithArgs(1).
			WillReturnRows(sqlmock.NewRows([]string{"id", "from_where", "destination", "flight_time", "flight_duration"}).
				AddRow(1, newFlight.FromWhere, newFlight.Destination, newFlight.FlightTime, newFlight.FlightDuration))

		flight, err := storage.Insert(context.Background(), newFlight)
		assert.NoError(t, err)
...
func TestUpdate(t *testing.T) {
	...
	t.Run("UpdateError", func(t *testing.T) {
		mock.ExpectExec("UPDATE " + os.Getenv("PSQL_TABLE_NAME") + " SET fromWhere = \\$1, destination = \\$2, flightTime = \\$3, flightDuration = \\$4 WHERE id = \\$5").
			WithArgs("NYC", "LAX", time.Now(), 6, 1).
			WillReturnError(fmt.Errorf("update error"))

		_, err := storage.Update(context.Background(), models.Flight{Id: 1, FromWhere: "NYC", Destination: "LAX", FlightDuration: 6, FlightTime: time.Now()})
		assert.Error(t, err)
